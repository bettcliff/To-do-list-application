...
class WeeklyTaskTracker:
    ...
    def create_dashboard(self):
        ...
        self.fig = plt.Figure(figsize=(6, 6), facecolor=self.colors['chart_bg'])
        self.ax = self.fig.add_subplot(111)
        
        # Configure pie chart
        self.ax.set_facecolor(self.colors['chart_bg'])
        
        # Embed chart in dashboard frame
    ...
    def update_dashboard(self):
        ...
        for i, row_vars in enumerate(self.task_vars):
            ...
            if task_text:  # Only show tasks with descriptions
                ...
                task_data.append(checked)
                labels.append(task_text)
        
        if task_data:  # Only create chart if there are tasks
            # Create pie chart
            colors = plt.cm.Set3(np.linspace(0, 1, len(task_data)))
            self.ax.pie(task_data, labels=labels, colors=colors, autopct='%1.1f%%',
                       textprops={'color': self.colors['text']})
            self.ax.set_title('Task Distribution', color=self.colors['text'])
