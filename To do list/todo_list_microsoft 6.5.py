...
class WeeklyTaskTracker:
    ...
    def create_header(self):
        ...
        end_date_label.pack(side="left", padx=(0, 5))
    ...
    def get_month_text(self):
        ...
        if start_date.month == end_date.month:
            return f"Month: {start_date.strftime('%B %Y')}"
        else:
            return f"Months: {start_date.strftime('%B')} - {end_date.strftime('%B %Y')}"
        
    def get_week_text(self):
        start_date = self.start_date_picker.get_date()
        end_date = self.end_date_picker.get_date()
        start_week = (start_date.day - 1) // 7 + 1
        end_week = (end_date.day - 1) // 7 + 1
        if start_week == end_week:
            return f"Week {start_week}"
        else:
