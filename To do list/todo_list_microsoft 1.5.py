...
class CategoryPage(ttk.Frame):
    ...
    def create_tasks_list(self):
        ...
        if self.category_name == "My Day":
            ttk.Label(list_frame, text="Active Tasks", font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0,10))
    ...
    def create_completed_tasks_list(self):
        ...
        completed_frame = ttk.Frame(self)
        completed_frame.pack(fill="both", expand=True, padx=20, pady=(20,0))

        # Label for completed tasks
        ttk.Label(completed_frame, text="Completed Tasks", font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(0,10))
    ...
    def edit_task(self):
        ...
        if selected:
            ...
            task_entry.insert(0, values[1])  # Task is now in second column
            task_entry.pack(pady=5)
            
            ttk.Label(edit_window, text="Due Date:").pack(pady=5)
            due_date = DateEntry(edit_window)
            due_date.set_date(values[2])  # Due date is now in third column
            due_date.pack(pady=5)
            
            def save_changes():
                for task in self.main_app.tasks:
                    if (task["category"] == self.category_name and 
                        task["task"] == values[1]):  # Task is now in second column
