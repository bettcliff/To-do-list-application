    ...
    def add_task_reward_pair(self):
        ...
        if empty_row is not None:
            ...
            add_button = ttk.Button(
                pair_frame,
                text="Add to Tasks",
                command=lambda: self.add_to_main_tasks(task_entry, reward_entry, pair_frame, empty_row)
            )
            add_button.pack(side="left", padx=5)

    def add_to_main_tasks(self, task_entry, reward_entry, pair_frame, row):
        # Get task text
        task_text = task_entry.get()
        if task_text and task_text != "Enter task...":
            # Add to main task grid
            ...
            # Check all checkboxes for the task
            for var in self.task_vars[row]:
                var.set(True)
            
            # Update progress
            self.update_progress(row+1)
            
            # Clear and destroy the pair frame
            pair_frame.destroy()
