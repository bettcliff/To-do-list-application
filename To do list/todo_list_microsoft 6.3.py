...
class WeeklyTaskTracker:
    ...
    def create_layout(self):
        ...
        rewards_frame = ttk.Frame(nav_frame, style="Tracker.TFrame")
        rewards_frame.pack(side="right", padx=5)

        # Create a frame for task-reward pairs
        self.reward_pairs_frame = ttk.Frame(rewards_frame, style="Tracker.TFrame")
        self.reward_pairs_frame.pack(side="right")
    ...
    def create_history_section(self):
        ...
        scrollbar = ttk.Scrollbar(self.history_page, orient="vertical", command=history_canvas.yview)
    ...
    def update_dashboard(self):
        ...
        if task_names:  # Only create chart if there are tasks
            ...
            # Add text labels under the first bar
            for i, (name, prog) in enumerate(zip(task_names, progress)):
                if i > 0:  # Skip first bar
                    self.ax1.text(prog/2, i, f'{prog:.1f}%', 
                                ha='center', va='center',
